...
app.showCart=function(){
	...
	if(list.length===0){
		...
		for(let i=0;i<list.length;i++){
			let data=list[i];
			let item=app.createElement("div", {atrs:{
				className:"row"
			}}, container);
			// variant
			let variant=app.createElement("div", {atrs:{
				className:"variant"
			}}, item);
			let picture=app.createElement("div", {atrs:{
				className:"picture"
			}}, variant);
			...
			let qtySelector=app.createElement("select", {atrs:{
				index:i
			}, evts:{
				change:app.evts.changeQty
			}}, qty);
			for(let j=1;j<=data.stock;j++){
				qtySelector.add(app.createElement("option", {atrs:{
					value:j, textContent:j
				}}));
			}
			qtySelector.selectedIndex=data.qty-1;
			// price
			app.createElement("div", {atrs:{
				className:"price", textContent:"NT. "+data.price
			}}, item);
			// subtotal
			app.createElement("div", {atrs:{
				className:"subtotal", textContent:"NT. "+(data.price*data.qty)
			}}, item);
			// remove
			app.createElement("img", {atrs:{
				src:"imgs/cart-remove.png", index:i
			}}, app.createElement("div", {atrs:{
				className:"remove", index:i
			}, evts:{
