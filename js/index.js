...
app.showKeyvisual=function(keyvisual){
	...
	keyvisual.data.forEach(function(item, index){
		// create circle
		let circle=app.createElement("a", {atrs:{
			className:"circle"+(index===0?" current":""), index:index
		}, evts:{
	...
	app.evts.clickKeyvisual=function(e){
		...
		app.get("#keyvisual>.visual:nth-child("+(keyvisual.step+1)+")").classList.remove("current");
		app.get("#keyvisual>.step>.circle:nth-child("+(keyvisual.step+1)+")").classList.remove("current");
		keyvisual.step=step;
		app.get("#keyvisual>.visual:nth-child("+(keyvisual.step+1)+")").classList.add("current");
